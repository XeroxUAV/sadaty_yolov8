...
class WindowDetector:
    def __init__(self,  send_text,model_path=r"C:\softwares\yolov8_env64\runs\segment\window_segmentation_model8\weights\best.pt",
                 conf_thresh=0.6):
    ...
    def perform_search(self):
        ...
        if not self.first_search_done:
            ...
            self.send_text(f"search_yaw_speed: {self.search_direction * self.search_yaw_speed}")
            print(f"search_yaw_speed: {self.search_direction * self.search_yaw_speed}")
            self.send_text(f"search_yaw_speed: {self.search_direction * self.search_yaw_speed}")
        else:
            delta_angle = self.search_direction * self.search_yaw_speed * dt
            self.search_yaw_angle += delta_angle
            ...
            self.send_text(f"search_throttle_speed: {throttle_change} , search_yaw_speed: {yaw_change}")
            print(f"search_throttle_speed: {throttle_change} , search_yaw_speed: {yaw_change}")
            self.send_text(f"search_throttle_speed: {throttle_change} . search_yaw_speed: {yaw_change}")
    ...
    def process(self, frame, pid_yaw, pid_throttle, pid_roll, pid_forward):
        ...
        if approx_poly is not None and final_mask_bin is not None:
            ...
            if centered_horizontally and centered_vertically and yaw_center and not self.forward_done:
                ...
                print("(0, 20, 0, 0)")
                self.send_text("(0, 20, 0, 0)")
                print("forward time" , forward_time)
                self.send_text(f"forward time:{ forward_time}")
                ...
                while time.time() - start_time < forward_time:
                    ...
                    if key == ord('q') or keyboard.is_pressed('q'):
                        ...
                        # tello.send_rc_control(0, 0, 0, 0)
                        # tello.land()
                        print("tello.send_rc_control(0, 0, 0, 0)tello.land()")
                        break
            ...
            self.send_text(f"yaw_speed: {yaw_speed}" )
            print("yaw_error:", yaw_error)
            self.send_text(f"yaw_error:{yaw_error}" )
            throttle = int(np.clip(pid_throttle.update(-error_y), -30, 30)) if abs(error_y) > 7 else 0
            print("error_y" , error_y)
            self.send_text(f"error_y:{error_y}"  )


            print(f"error_x:{error_x}" )
            self.send_text(f"error_x:{ error_x}")
            if abs(error_x) > 7:
            ...
            if hasattr(self, "prev_area"):
                delta_area = self._compute_area_rate(best_area, now)
                print(f"Delta area: {delta_area}, Forward_RC: {forward_backward_speed}")
                self.send_text(f"Delta area:{delta_area}" )
                self.send_text(f"Forward_RC:{ forward_backward_speed}")
            ...
            try:
                print(f"({roll_speed}, {forward_backward_speed}, {throttle}, {yaw_speed})")
                self.send_text(f"({roll_speed}, {forward_backward_speed}, {throttle}, {yaw_speed})")
            except:
                print("_")
                self.send_text("_")
...
class DroneController:
    def __init__(self):
        ...
        self.detector = WindowDetector(self.send_text)
